...
do
  ...
  local ext = reaper.GetExtState("UniversalSeventh", "last_time")
  local last = (ext == nil or ext == "") and 0 or (tonumber(ext) or 0)
  if last > 0 and (now - last) < 0.2 then return end
  reaper.SetExtState("UniversalSeventh", "last_time", tostring(now), false)
end
...
local r, int3, int5, int7, quality
if scale_enabled then
  r = steps[deg_idx]
  local i3 = ((deg_idx + 2 - 1) % #steps) + 1
  local i5 = ((deg_idx + 4 - 1) % #steps) + 1
  local i7 = ((deg_idx + 6 - 1) % #steps) + 1
  local t3 = steps[i3]
  local t5 = steps[i5]
  local t7 = steps[i7]
  if i3 <= deg_idx then t3 = t3 + 12 end
  if i5 <= deg_idx then t5 = t5 + 12 end
  if i7 <= deg_idx then t7 = t7 + 12 end
  int3 = mod12(t3 - r)
  int5 = mod12(t5 - r)
  int7 = mod12(t7 - r)
  if int3 == 4 and int5 == 7 and int7 == 11 then quality = "major seventh"
  elseif int3 == 3 and int5 == 7 and int7 == 10 then quality = "minor seventh"
  elseif int3 == 4 and int5 == 7 and int7 == 10 then quality = "dominant seventh"
  elseif int3 == 3 and int5 == 6 and int7 == 10 then quality = "half-diminished seventh"
  elseif int3 == 3 and int5 == 6 and int7 == 9 then quality = "diminished seventh"
  elseif int3 == 3 and int5 == 7 and int7 == 11 then quality = "minor-major seventh"
  elseif int3 == 4 and int5 == 8 and int7 == 11 then quality = "augmented-major seventh"
  else quality = "seventh (undefined)" end
else
  int3 = 4
  int5 = 7
  quality = "major triad"
...
if not root_pitch_ins or not root_sppq or not root_eppq then reaper.PreventUIRefresh(-1) reaper.Undo_EndBlock2(0, "Insert chord: canceled", -1) return end
...
local seventh_pitch = scale_enabled and (root_pitch_ins + int7) or nil
...
if scale_enabled and seventh_pitch then
  reaper.MIDI_InsertNote(take, true, false, root_sppq, root_sppq + len_ppq, root_chan_ins or (sel_chan or 0), seventh_pitch, root_vel_ins or (sel_vel or 96), false)
end
reaper.MIDI_Sort(take)
reaper.PreventUIRefresh(-1)
local undo_desc = scale_enabled and ("Insert seventh: " .. quality) or ("Insert triad: " .. quality)
